...
| /api/documents/{id}/ | PATCH | required | owner, admin | 403 non-owner | ok | status field ignored | 200 | 403 | status manipulation ignored, approved-doc edit blocked, row locked during edit | select_for_update | test_documents.py, test_audit_regressions.py | API.md | phase2, audit |
| /api/documents/{id}/ | DELETE | required | owner(draft), admin | 403 non-draft for creator | ok | n/a | 204 | 403 | non-owner delete blocked, files removed after commit | atomic | test_documents.py, test_audit_regressions.py | API.md | phase2, audit |
...
| /api/documents/{id}/reject/ | POST | required | reviewer, admin | 404 outside scope | ok | empty/whitespace/non-string reason | 200 | 400/403/404 | creator blocked, reason required | select_for_update | test_workflow.py, test_audit_regressions.py | API.md | phase5, audit |
| /api/documents/{id}/request-changes/ | POST | required | reviewer, admin | 404 outside scope | ok | empty/non-string comment | 200 | 400/403/404 | comment required | select_for_update | test_workflow.py, test_audit_regressions.py | API.md | phase5, audit |
...
| /api/documents/{id}/versions/{version_id}/ | GET | required | any with doc access | 404 wrong parent doc | ok | non-numeric version_id | 200 | 404 | version scoped to parent doc | n/a | test_versions.py, test_audit_regressions.py | API.md | phase7, audit |
| /api/documents/{id}/versions/{version_id}/download/ | GET | required | any with doc access | 404 wrong parent doc | ok | non-numeric version_id | 200 | 404 | version scoped to parent doc | n/a | test_versions.py, test_audit_regressions.py | API.md | phase7, audit |
